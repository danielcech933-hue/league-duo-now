...
import { Check, Loader2, RefreshCw, Shield, Sparkles, Trophy, X } from "lucide-react";
...
function Stat({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl bg-black/20 p-3"><div className="text-[10px] uppercase tracking-wider text-slate-600">{label}</div><div className="mt-1 font-bold text-slate-300">{value}</div></div>;
}

export function RiotConnect({ onChanged }: { onChanged?: (account: RiotAccount | null) => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!userId) return;
      const { data } = await supabase.from("riot_accounts").select("*").eq("user_id", userId).maybeSingle();
  ...
  const sync = async () => {
    ...
    const { data, error: invokeError } = await supabase.functions.invoke("riot-sync", {
      body: { gameName: gameName.trim(), tagLine: tagLine.trim().replace(/^#/, ""), region },
    });
    ...
    else if (data?.account) { setAccount(data.account as RiotAccount); onChanged?.(data.account as RiotAccount); }
  ...
  const disconnect = async () => {
    ...
    setAccount(null); setGameName(""); setTagLine(""); onChanged?.(null);
  };
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
      <section className="rounded-[30px] border border-white/[.07] bg-white/[.025] p-7 sm:p-9">
        <div className="flex items-center gap-3">
          <div className="grid h-12 w-12 place-items-center rounded-2xl bg-red-500/10 text-red-300"><Shield /></div>
          <div>
            <div className="text-xs font-bold uppercase tracking-widest text-violet-300">Verified identity</div>
            <h2 className="text-2xl font-black">Connect Riot ID</h2>
          </div>
        </div>
        <p className="mt-5 max-w-xl text-sm leading-6 text-slate-500">Your Riot ID becomes your identifier in matchmaking — teammates see it on your card together with your verified rank, level and top champions.</p>
      ...
      <aside className="rounded-[30px] border border-white/[.07] bg-white/[.025] p-6">
        ...
        {!account ? <div className="py-12 text-center"><Trophy className="mx-auto text-slate-700" size={42} /><div className="mt-4 font-bold">Not connected</div><p className="mt-1 text-sm text-slate-600">Verify a Riot ID to see your ranked profile.</p></div> : <>
          <div className="mt-6 flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-emerald-400/10 text-emerald-300"><Check /></div><div><div className="font-black">{account.game_name}#{account.tag_line}</div><div className="text-xs text-emerald-300">{account.verified ? "Verified" : "Unverified"} · {account.region}</div></div></div>
