...
import { Award, Check, Copy, Gamepad2, Globe2, History, MessageCircle, Shield, Star, Swords, Trophy, UserRound, X } from "lucide-react";
...
export function PlayerProfileModal({ userId, name, onClose, onToast }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setRiot((ra as RiotAccount | null) || null);
  ...
  const copyRiotId = async () => {
    if (!riot) return;
    await navigator.clipboard?.writeText(`${riot.game_name}#${riot.tag_line}`);
    onToast?.("Riot ID copied.", "success");
  };

  const recordGame = async (result: "win" | "loss" | "unknown") => {
    setGameBusy(true);
    const { data, error: e } = await supabase.rpc("record_duo_game", { _other: userId, _result: result, _game_mode: "ranked_solo" });
    setGameBusy(false);
    if (e) { onToast?.(e.message, "error"); return; }
    if (data) {
      ...
      <div className="px-6 pb-7 sm:px-8">
        <div className="-mt-10 flex items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            {connected === "accepted" ? <><span className="inline-flex items-center gap-1.5 rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-300"><Check size={14}/> Connected</span><button onClick={invite} disabled={inviteBusy} className="inline-flex items-center gap-1.5 rounded-xl bg-violet-500 px-3 py-2 text-xs font-black text-white disabled:opacity-50"><Gamepad2 size={14}/> {inviteBusy ? "Sending…" : "Play again"}</button></> : connected === "pending" && requestByMe ? <span className="rounded-xl border border-white/10 px-3 py-2 text-xs font-bold text-slate-500">Request sent</span> : <button onClick={add} disabled={connectionBusy || connected === "pending"} className="inline-flex items-center gap-1.5 rounded-xl bg-white px-3 py-2 text-xs font-black text-slate-950 disabled:opacity-50"><UserRound size={14}/> {connectionBusy ? "Adding…" : "Add teammate"}</button>}
          </div>
        </div>
        {busy ? <div className="py-12 text-center text-sm text-slate-600">Loading profile…</div> : profile ? <>
          ...
          {riot && <div className="mt-5 rounded-2xl border border-violet-400/15 bg-violet-400/[.05] p-4"><div className="flex items-center justify-between gap-3"><div><div className="text-[10px] font-bold uppercase tracking-widest text-violet-300">Riot ID</div><div className="mt-1 flex flex-wrap items-center gap-2 text-lg font-black text-white">{riot.game_name}<span className="text-slate-500">#{riot.tag_line}</span>{riot.verified && <span className="rounded-full bg-emerald-400/10 px-2 py-1 text-[9px] font-black text-emerald-300">Verified</span>}</div><div className="mt-1 text-xs text-slate-500">{riot.region}{riot.rank_tier ? ` · ${riot.rank_tier} ${riot.rank_division || ""}` : ""}</div></div><button onClick={copyRiotId} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 text-slate-400 hover:text-white" aria-label="Copy Riot ID"><Copy size={15}/></button></div><p className="mt-3 text-xs text-slate-600">Use this Riot ID to add the teammate directly in League of Legends.</p></div>}
          ...
          <div className="mt-5 rounded-2xl border border-white/[.06] bg-black/20 p-4">
            <div className="flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-bold"><History size={15} className="text-violet-300"/> Shared game history</div><span className="text-xs text-slate-500">{lastPlayed ? `Last played ${lastPlayed}` : "No recorded games yet"}</span></div>
            <div className="mt-4 grid grid-cols-3 gap-2"><Stat label="Played together" value={String(history?.played_together ?? 0)}/><Stat label="Wins" value={String(history?.wins ?? 0)}/><Stat label="Losses" value={String(history?.losses ?? 0)}/></div>
            {canRecord ? <div className="mt-4 flex flex-wrap gap-2 border-t border-white/[.06] pt-4"><button disabled={gameBusy} onClick={() => void recordGame("win")} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-300 disabled:opacity-50"><Trophy size={13}/> Record win</button><button disabled={gameBusy} onClick={() => void recordGame("loss")} className="inline-flex items-center gap-1.5 rounded-lg bg-red-400/10 px-3 py-2 text-xs font-black text-red-300 disabled:opacity-50"><Swords size={13}/> Record loss</button><button disabled={gameBusy} onClick={() => void recordGame("unknown")} className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-2 text-xs font-bold text-slate-400 disabled:opacity-50"><Gamepad2 size={13}/> Record game</button></div> : <p className="mt-4 text-[11px] leading-5 text-slate-600">Game history can be recorded after you are connected with this teammate.</p>}
          </div>
